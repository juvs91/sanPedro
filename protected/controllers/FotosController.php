...
class FotosController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{               
			...
			$album = Albums::model()->findByPk($model->idAlbum);
			
		  	$model->image=CUploadedFile::getInstance($model,'image');   
		
		 	
			if ($model->image == null) {
		   		throw new CHttpException(500, 'tienes que seleccionar un archivo'); 
			}
			$model->url='protected/fotos/albums/'.$album->nombre."/".$model->image->name;
			
			if(!file_exists("protected/fotos/albums/".$album->nombre))                    
			...
			if($model->save()){
				 ...
				 $model->image->saveAs('protected/fotos/albums/'.$album->nombre."/".$model->image);
	...
	{
		$dataProvider=Fotos::model()->findAll();
